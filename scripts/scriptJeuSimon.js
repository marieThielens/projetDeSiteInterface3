...
var secondString2Sound = new Audio("https://www.freesound.org/data/previews/8/8389_6479-lq.ogg");
...
function isUserSequenceCorrect() {
  if (userSequence.length == simonSequence.length) {
    if (compareArrays(userSequence,simonSequence)) {
      if (simonSequence.length < 20) {
        ...
        youWon();
          ...
          setTimeout(function(){
            $('#info').html('Desolé, tu t es trompé.');
            if (strictMode) {
              simonSequence=[];
              simonSequence.push(getRandomString());
            }
    ...
    if (!compareArrays(userSequence, 
      ...
      setTimeout(function(){
          $('#info').html('désolé, tu t es trompé.');
        if (strictMode) {
              simonSequence=[];
              simonSequence.push(getRandomString());
            }
